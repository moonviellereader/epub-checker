"""
EPUB Simple Diff - Highlight Perbedaan dengan Warna Biru
Cukup upload 2 EPUB, bagian yang berbeda akan di-highlight biru
"""
...
st.set_page_config(page_title="EPUB Diff", page_icon="📚", layout="wide")
...
with col1:
    epub1 = st.file_uploader("📖 EPUB Lama", type=['epub'], key='e1')
with col2:
    epub2 = st.file_uploader("📖 EPUB Baru", type=['epub'], key='e2')

# Sidebar
show_same = st.sidebar.checkbox("Tampilkan yang sama", value=False)
only_diff = st.sidebar.checkbox("Hanya chapter berbeda", value=True)
        ...
        for i, (idx1, idx2) in enumerate(matched):
            ...
            if idx1 is None:
                ...
                if only_diff or selected != "-- Semua --":
                    st.markdown(f'<div class="chapter-header">🆕 {html.escape(ch["title"])} (Baru)</div>', unsafe_allow_html=True)
                    rows = []
            ...
            if idx2 is None:
                ...
                ch = chapters1[idx1]
                if only_diff or selected != "-- Semua --":
                    st.markdown(f'<div class="chapter-header">❌ {html.escape(ch["title"])} (Dihapus)</div>', unsafe_allow_html=True)
                    rows = []
                    for j, p in enumerate(ch['paragraphs']):
                        rows.append(f'''<div class="row">
